...
pub fn create_dynamodb_helper(item: TokenStream) -> TokenStream {
    ...
    let from_struct_for_hashmap = tokenstream_or_empty_if_no_put_methods(
        from_struct_for_hashmap(&name, fields), &exclusion_list_refs
    );

    let try_from_hashmap_for_struct = tokenstream_or_empty_if_no_retrieval_methods(
        try_from_hashmap_to_struct(&name, &parse_error, fields), &exclusion_list_refs,
    );

    let new = tokenstream_or_empty_if_exclusion(
        new_method(&helper_ident), NEW_METHOD_NAME, &exclusion_list_refs,
    );

    let build = tokenstream_or_empty_if_exclusion(
        build_method(&helper_ident), BUILD_METHOD_NAME, &exclusion_list_refs,
    );

    let gets = tokenstream_or_empty_if_exclusion(
        get_methods(&name, &get_error, &get_by_partition_error, partition_key_ident_and_type, range_key_ident_and_type), GET_METHOD_NAME, &exclusion_list_refs
    );

    let batch_get = tokenstream_or_empty_if_exclusion(
        batch_get(&name, &batch_get_error, partition_key_ident_and_type, range_key_ident_and_type), BATCH_GET_METHOD_NAME, &exclusion_list_refs
    );

    let create_table = tokenstream_or_empty_if_exclusion(
        create_table_method(partition_key_ident_and_type, range_key_ident_and_type), CREATE_TABLE_METHOD_NAME, &exclusion_list_refs
    );
    let delete_table = tokenstream_or_empty_if_exclusion(
        delete_table_method(), DELETE_TABLE_METHOD_NAME, &exclusion_list_refs
    );
    let put = tokenstream_or_empty_if_exclusion(
        put_method(&name), PUT_METHOD_NAME, &exclusion_list_refs,
    );
    let batch_put = tokenstream_or_empty_if_exclusion(
        batch_put_method(&name), BATCH_PUT_METHOD_NAME, &exclusion_list_refs,
    );
    let delete = tokenstream_or_empty_if_exclusion(
        delete_method(&name, partition_key_ident_and_type, range_key_ident_and_type), DELETE_METHOD_NAME, &exclusion_list_refs,
    );
    let scan = tokenstream_or_empty_if_exclusion(
        scan_method(&name, &scan_error), SCAN_METHOD_NAME, &exclusion_list_refs,
    );
